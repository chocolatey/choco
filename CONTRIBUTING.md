...
A commit message should observe the following:
  ...
  * The first line of the commit message should be a short description around 50 characters in length and be prepended with the Github issue it refers to with parentheses surrounding that. If the github issue is #25, you should have `(GH-25)` prepended to the message.
  * If the commit is about documentation, the message should be prepended with `(doc)`.
  * If it is a trivial commit or one of formatting/spaces fixes, it should be prepended with `(maint)`.
