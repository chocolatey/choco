...
namespace chocolatey.infrastructure.app.builders
{
    ...
    public static class ConfigurationBuilder
    {
        ...
        private static void SetAllConfigItems(ChocolateyConfiguration config, ConfigFileSettings configFileSettings, IFileSystem fileSystem)
        {
            ...
            {
                SetConfigItem(ApplicationParameters.ConfigSettings.CommandExecutionTimeoutSeconds, configFileSettings, ApplicationParameters.DefaultWaitForExitInSeconds.ToStringSafe(), "Default timeout for command execution. '0' for infinite (starting in 0.10.4). It is recommended that organizations bump this up to at least 4 hours (14400).", forceSettingValue: true);
            ...
            if (webRequestTimeoutSeconds <= 0)
            {
                ...
                SetConfigItem(ApplicationParameters.ConfigSettings.WebRequestTimeoutSeconds, configFileSettings, ApplicationParameters.DefaultWebRequestTimeoutInSeconds.ToStringSafe(), "Default timeout for web requests. Available in 0.9.10+.", forceSettingValue: true);
            }
                ...
                (optionSet) =>
                {
                    optionSet
                        ...
                        .Add("timeout=|execution-timeout=",
                             "CommandExecutionTimeout (in seconds) - The time to allow a command to finish before timing out. Overrides the default execution timeout in the configuration of {0} seconds. '0' for infinite starting in 0.10.4.".FormatWith(config.CommandExecutionTimeoutSeconds.ToStringSafe()),
                            option =>
                            {
                                int timeout = 0;
