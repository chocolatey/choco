...
namespace chocolatey.infrastructure.app.commands
{
    ...
    [CommandFor("rules", "view or list implemented package rules (alias for rule)", Version = "2.3.0")]
    public class ChocolateyRuleCommand : ChocolateyCommandBase, ICommand
    {
        ...
        public virtual void ConfigureArgumentParser(OptionSet optionSet, ChocolateyConfiguration configuration)
        {
            optionSet
                .Add("n=|name=",
                "The name or identifier of the rule to show more details about. Required with actions other than list. Defaults to empty.",
        ...
        public virtual void Run(ChocolateyConfiguration config)
        {
            ...
            if (!string.IsNullOrEmpty(config.RuleCommand.Name))
            {
                ...
                if (config.RegularOutput)
                {
                    ...
                    {
                        throw new ApplicationException("No rule with the identifier {0} could be found.".FormatWith(config.RuleCommand.Name));
                    }

                    // Not using multiline logging here, as it causes issues
                    // with unit tests.
                    this.Log().Info("ID: {0} | Severity: {1}", foundRule.Id, foundRule.Severity);
        ...
        public virtual void Validate(ChocolateyConfiguration configuration)
        {
            switch (configuration.RuleCommand.Command)
            {
                case "list":
                    ...
                    {
                        throw new ApplicationException("A Rule Name can not be specified when listing all validation rules.");
                    }
                    break;
                case "get":
                    if (string.IsNullOrEmpty(configuration.RuleCommand.Name))
                    {
                        throw new ApplicationException("A Rule Name is required to when not listing all validation rules.");
                    }
                    break;
        ...
        {
            return @"Get or list information about what rule validations is implemented by Chocolatey CLI
