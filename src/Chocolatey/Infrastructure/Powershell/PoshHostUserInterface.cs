...
namespace Chocolatey.Infrastructure.Powershell
{
  ...
  Respond within {0} seconds or the default selection will be chosen.".FormatWith(TimeoutInSeconds));
            ...
            return secureStringPlainText.ToSecureString();
        }
        ...
        public override void WriteProgress(long sourceId, ProgressRecord record)
        {
            ...
            if (_configuration.Features.ShowDownloadProgress)
            {
                ...
                Console.Write("\rProgress: {0}% - {1}".FormatWith(record.PercentComplete.ToStringChecked(), record.StatusDescription).PadRight(Console.WindowWidth));
            }
        }
        ...
        private static string[] GetHotkeyAndLabel(string input)
        {
            ...
            {
                if (fragments[1].Length > 0) result[0] = fragments[1][0].ToStringChecked().ToUpper(CultureInfo.CurrentCulture);
