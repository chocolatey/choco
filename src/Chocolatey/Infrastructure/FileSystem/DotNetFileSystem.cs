    ...
    public sealed class DotNetFileSystem : IFileSystem
    {
        ...
        public string CombinePaths(string leftItem, params string[] rightItems)
        {
            ...
            foreach (var rightItem in rightItems)
            {
                ...
                if (rightSide.StartsWith(Path.DirectorySeparatorChar.ToStringChecked()) || rightSide.StartsWith(Path.AltDirectorySeparatorChar.ToStringChecked()))
                {
        ...
        public string GetExecutablePath(string executableName)
        {
            ...
            foreach (var path in searchPaths.OrEmpty())
            {
                ...
                {
                    var possiblePath = CombinePaths(path, "{0}{1}".FormatWith(executableName, extension.ToLowerChecked()));
        ...
        public bool IsEncryptedFile(dynamic file)
        {
            ...
            string fullName = file.FullName;
            this.Log().Debug(ChocolateyLoggers.Verbose, () => "Is file \"{0}\" an encrypted file? {1}".FormatWith(fullName, isEncrypted.ToStringChecked()));
            return isEncrypted;
        }
        ...
        {
            EnsureDirectory(GetDirectoryName(newFilePath), ignoreError: true);
        ...
        public void CopyFile(string sourceFilePath, string destinationFilePath, bool overwriteExisting)
        {
            ...
            EnsureDirectory(GetDirectoryName(destinationFilePath), ignoreError: true);
        ...
        public void MoveDirectory(string directoryPath, string newDirectoryPath)
        {
            ...
            catch (Exception ex)
            {
                ...
                EnsureDirectory(newDirectoryPath, ignoreError: true);
                foreach (var file in GetFiles(directoryPath, "*.*", SearchOption.AllDirectories).OrEmpty())
                {
                    ...
                    EnsureDirectory(GetDirectoryName(destinationFile), ignoreError: true);
        ...
        {
            EnsureDirectory(destinationDirectoryPath, ignoreError: true);
        ...
        {
            EnsureDirectory(directoryPath, false);
        }

        public void EnsureDirectory(string directoryPath, bool ignoreError)
        {
            if (!DirectoryExists(directoryPath))
            {
                try
        ...
        public void EnsureFileAttributeSet(string path, FileAttributes attributes)
        {
            if (DirectoryExists(path))
            {
                ...
                {
                    this.Log().Debug(ChocolateyLoggers.Verbose, () => "Adding '{0}' attribute(s) to '{1}'.".FormatWith(attributes.ToStringChecked(), path));
        ...
        public void EnsureFileAttributeRemoved(string path, FileAttributes attributes)
        {
            if (DirectoryExists(path))
            {
                ...
                if ((directoryInfo.Attributes & attributes) == attributes)
                {
                    this.Log().Debug(ChocolateyLoggers.Verbose, () => "Removing '{0}' attribute(s) from '{1}'.".FormatWith(attributes.ToStringChecked(), path));
                    directoryInfo.Attributes &= ~attributes;
                }
            }
            if (FileExists(path))
