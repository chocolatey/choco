...
namespace Chocolatey.Infrastructure.Results
{
    ...
    public sealed class PackageResult : Result
    {
        ...
        public void ResetMetadata(IPackageMetadata metadata, IPackageSearchMetadata search)
        {
            ...
            Name = metadata.Id;
            Version = metadata.Version.ToStringChecked();
        }

        public PackageResult(IPackageMetadata packageMetadata, string installLocation, string source = null) : this(packageMetadata.Id, packageMetadata.Version.ToStringChecked(), installLocation)
        {
            PackageMetadata = packageMetadata;
            Source = source;
        }

        public PackageResult(IPackageSearchMetadata packageSearch, string installLocation, string source = null) : this(packageSearch.Identity.Id, packageSearch.Identity.Version.ToStringChecked(), installLocation)
        {
            ...
            if (!string.IsNullOrEmpty(source))
            {
                ...
                {
                    this.Log().Debug("Unable to determine sources from '{0}'. Using value as is.{1} {2}".FormatWith(source, Environment.NewLine, ex.ToStringChecked()));
                    // source is already set above
                    return;
                }
            }

            Source = sources.FirstOrDefault(uri => uri.IsFile || uri.IsUnc).ToStringChecked();
            /*
        ...
        public PackageResult(IPackageMetadata packageMetadata, IPackageSearchMetadata packageSearch, string installLocation, string source = null) : this(packageMetadata.Id, packageMetadata.Version.ToStringChecked(), installLocation)
        {
            SearchMetadata = packageSearch;
            PackageMetadata = packageMetadata;
            var sources = new List<Uri>();
