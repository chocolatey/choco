    ...
    public sealed class VersionInformation
    {
        public static string GetCurrentAssemblyVersion(IAssembly assembly = null)
        {
            ...
            if (string.IsNullOrEmpty(version))
            {
                ...
                {
                    if (attribute.ToStringChecked().Contains("AssemblyFileVersion"))
                    {
                        version = attribute.ConstructorArguments[0].Value.ToStringChecked();
                        break;
                    }
                }
            }
        ...
        public static string GetCurrentInformationalVersion(IAssembly assembly = null)
        {
            ...
            if (string.IsNullOrEmpty(version))
            {
                ...
                foreach (var attribute in attributes)
                {
                    if (attribute.ToStringChecked().Contains("AssemblyInformationalVersion"))
                    {
                        version = attribute.ConstructorArguments[0].Value.ToStringChecked();
                        break;
                    }
        ...
        public static string GetMinimumChocolateyVersion(IAssembly assembly = null)
        {
            ...
            {
                if (attribute.ToStringChecked().Contains("MinimumChocolateyVersion"))
                {
                    return attribute.ConstructorArguments[0].Value.ToStringChecked();
                }
            }
