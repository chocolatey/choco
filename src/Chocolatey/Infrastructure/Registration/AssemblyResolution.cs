...
namespace Chocolatey.Infrastructure.Registration
{
    ...
    public class AssemblyResolution
    {
            ...
            if (lockTaken)
            {
                try
                {
                    ...
                    if (resolvedAssembly == null)
                    {
                        ...
                        if (string.IsNullOrWhiteSpace(publicKeyToken) || AssemblyExtensions.GetPublicKeyToken(tempAssembly.GetName()).IsEqualTo(publicKeyToken))
                        {
                            ...
                            _loadedAssemblies.TryAdd(assemblySimpleName.ToLowerChecked(), resolvedAssembly);
        ...
        private static IAssembly ResolveAssembly(string assemblySimpleName, string publicKeyToken)
        {
            ...
            {
                if (string.IsNullOrWhiteSpace(publicKeyToken) || AssemblyExtensions.GetPublicKeyToken(assembly.GetName()).IsEqualTo(publicKeyToken))
                {
            ...
            if (_loadedAssemblies.TryGetValue(assemblySimpleName.ToLowerChecked(), out tempAssembly))
            {
                if (string.IsNullOrWhiteSpace(publicKeyToken) || AssemblyExtensions.GetPublicKeyToken(tempAssembly.GetName()).IsEqualTo(publicKeyToken))
                {
                    "chocolatey".Log().Debug("Returning loaded assembly type for '{0}'".FormatWith(assemblySimpleName));
        ...
        public static System.Reflection.Assembly ResolveExtensionOrMergedAssembly(object sender, ResolveEventArgs args)
        {
            ...
            if (!AssemblyExtensions.GetPublicKeyToken(requestedAssembly).IsEqualTo(chocolateyPublicKey))
            {
