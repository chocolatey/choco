...
namespace Chocolatey.Infrastructure.App.Configuration
{
    ...
    public static class EnvironmentSettings
    {
        ...
        public static void SetEnvironmentVariables(ChocolateyConfiguration config)
        {
            ...
            Environment.SetEnvironmentVariable("OS_VERSION", config.Information.PlatformVersion.ToStringChecked());
            ...
            Environment.SetEnvironmentVariable("chocolateyRequestTimeout", config.WebRequestTimeoutSeconds.ToStringChecked() + "000");

            if (config.CommandExecutionTimeoutSeconds != 0)
            {
                Environment.SetEnvironmentVariable("chocolateyResponseTimeout", config.CommandExecutionTimeoutSeconds.ToStringChecked() + "000");
            }
 ...
 licensed version (choco upgrade chocolatey.extension).");
        ...
        public static void UpdateEnvironmentVariables()
        {
            ...
            var originalEnvironmentVariables = IntoCaseInsensitiveDictionary(Environment.GetEnvironmentVariables(EnvironmentVariableTarget.Process));
            var userName = originalEnvironmentVariables[ApplicationParameters.Environment.Username].ToStringChecked();
            var architecture = originalEnvironmentVariables[ApplicationParameters.Environment.ProcessorArchitecture].ToStringChecked();
            ...
            var machineVariables = IntoCaseInsensitiveDictionary(Environment.GetEnvironmentVariables(EnvironmentVariableTarget.Machine));
            var userVariables = IntoCaseInsensitiveDictionary(Environment.GetEnvironmentVariables(EnvironmentVariableTarget.User));
            ...
            var updatedPath = "{0};{1};".FormatWith(
                machineVariables[ApplicationParameters.Environment.Path].ToStringChecked(),
                userVariables[ApplicationParameters.Environment.Path].ToStringChecked()
                ).Replace(";;", ";");
            var updatedPathExt = "{0};{1};".FormatWith(
                machineVariables[ApplicationParameters.Environment.PathExtensions].ToStringChecked(),
                userVariables[ApplicationParameters.Environment.PathExtensions].ToStringChecked()
                ).Replace(";;", ";");
            var updatedPsModulePath = "{0};{1};".FormatWith(
                userVariables[ApplicationParameters.Environment.PsModulePath].ToStringChecked(),
                machineVariables[ApplicationParameters.Environment.PsModulePath].ToStringChecked()
                ).Replace(";;", ";");
        ...
        private static IDictionary IntoCaseInsensitiveDictionary(IDictionary originalDictionary)
        {
        ...
        private static void RefreshEnvironmentVariables(IDictionary environmentVariables)
        {
            ...
            {
                Environment.SetEnvironmentVariable(variable.Key.ToStringChecked(), variable.Value.ToStringChecked());
            }
        }
        ...
        private static string GetProcessOnlyItems(string currentValues, IEnumerable<string> originalValues)
        {
            ...
            return additionalItems.ToStringChecked();
        }
    }
}
