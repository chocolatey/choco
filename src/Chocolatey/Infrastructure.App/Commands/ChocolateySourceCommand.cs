...
namespace Chocolatey.Infrastructure.App.Commands
{
    ...
    public class ChocolateySourceCommand : IListCommand<ChocolateySource>
    {
        ...
        public virtual void Validate(ChocolateyConfiguration configuration)
        {
            ...
            {
                throw new ApplicationException("When specifying the subcommand '{0}', you must also specify --name.".FormatWith(configuration.SourceCommand.Command.ToStringChecked()));
            }

            if (configuration.SourceCommand.Command == SourceCommandType.Add && string.IsNullOrWhiteSpace(configuration.Sources))
            {
                throw new ApplicationException("When specifying the subcommand 'add', you must also specify --source.".FormatWith(configuration.SourceCommand.Command.ToStringChecked()));
            }
