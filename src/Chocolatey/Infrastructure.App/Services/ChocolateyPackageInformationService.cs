...
namespace Chocolatey.Infrastructure.App.Services
{
    ...
    public class ChocolateyPackageInformationService : IChocolateyPackageInformationService
    {
        ...
        public ChocolateyPackageInformation Get(IPackageMetadata package)
        {
            ...
            var pkgStorePath = _fileSystem.CombinePaths(ApplicationParameters.ChocolateyPackageInfoStoreLocation, "{0}.{1}".FormatWith(package.Id, package.Version.ToStringChecked()));
...
".FormatWith(_fileSystem.CombinePaths(pkgStorePath, RegistrySnapshotBadFile));
 ...
 ".FormatWith(_fileSystem.CombinePaths(pkgStorePath, RegistrySnapshotFile), _fileSystem.CombinePaths(pkgStorePath, RegistrySnapshotBadFile), e.ToString()));
        ...
        {
            _fileSystem.EnsureDirectory(ApplicationParameters.ChocolateyPackageInfoStoreLocation);
            ...
            var pkgStorePath = _fileSystem.CombinePaths(ApplicationParameters.ChocolateyPackageInfoStoreLocation, "{0}.{1}".FormatWith(packageInformation.Package.Id, packageInformation.Package.Version.ToStringChecked()));
            _fileSystem.EnsureDirectory(pkgStorePath);
            ...
            if (packageInformation.VersionOverride != null)
            {
                ...
                _fileSystem.WriteFile(versionOverrideFile, packageInformation.VersionOverride.ToStringChecked());
            }
            else
            {
