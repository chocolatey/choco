...
namespace Chocolatey.Infrastructure.App.Services
{
    ...
    public class PowershellService : IPowershellService
    {
        ...
        private string GetPackageScriptForAction(PackageResult packageResult, CommandNameType command)
        {
            ...
            var installScript = _fileSystem.GetFiles(packageDirectory, file, SearchOption.AllDirectories).Where(p => !p.ToLowerChecked().ContainsSafe("\\templates\\"));
 ...
 `choco -h` for details.");
        ...
        public void PreparePowerShellEnvironment(IPackageSearchMetadata package, ChocolateyConfiguration configuration, string packageDirectory)
        {
            ...
            Environment.SetEnvironmentVariable("chocolateyPackageVersion", package.Identity.Version.ToStringChecked());
            Environment.SetEnvironmentVariable("packageVersion", package.Identity.Version.ToStringChecked());
            Environment.SetEnvironmentVariable("chocolateyPackageVersionPrerelease", package.Identity.Version.Release.ToStringSafe());
            Environment.SetEnvironmentVariable("chocolateyPackageVersionPackageRelease", package.Identity.Version.Version.ToStringChecked());
                ...
                using (var pipeline = runspace.CreatePipeline())
                {
                    ...
                    pipeline.Output.DataReady += (sender, args) =>
                    {
                        ...
                        if (reader != null)
                        {
                            ...
                            {
                                host.UI.WriteLine(reader.Read().ToStringChecked().EscapeCurlyBraces());
                            }
                        }
                    };
                    ...
                    pipeline.Error.DataReady += (sender, args) =>
                    {
                        ...
                        if (reader != null)
                        {
                            while (reader.Count > 0)
                            {
                                host.UI.WriteErrorLine(reader.Read().ToStringChecked().EscapeCurlyBraces());
                            }
                        }
                    };
...
";
                    ...
                    catch (RuntimeException ex)
                    {
                        ...
                        if (record != null)
                        {
                            ...
                            {
                                var scriptError = scriptStackTrace.GetValue(record, null).ToStringChecked();
