...
".FormatWith(
                                ...
                                (package.DownloadCount == null || package.DownloadCount <= 0)  ? "n/a" : package.DownloadCount.ToStringChecked(),
                                (package.VersionDownloadCount == null || package.VersionDownloadCount <= 0) ? "n/a" : package.VersionDownloadCount.ToStringChecked(),
                                ...
                                !string.IsNullOrWhiteSpace(package.PackageSourceUrl.ToStringChecked())
                                    ? package.PackageSourceUrl.ToStringChecked()
                                    : "n/a",
                                ...
                                package.ProjectUrl != null ? package.ProjectUrl.ToStringChecked() : "n/a",
                                package.LicenseUrl != null && !string.IsNullOrWhiteSpace(package.LicenseUrl.ToStringChecked()) ? package.LicenseUrl.ToStringChecked() : "n/a",
                                !string.IsNullOrWhiteSpace(package.ProjectSourceUrl.ToStringChecked()) ? "{0} Software Source: {1}".FormatWith(Environment.NewLine, package.ProjectSourceUrl.ToStringChecked()) : string.Empty,
                                !string.IsNullOrWhiteSpace(package.DocsUrl.ToStringChecked()) ? "{0} Documentation: {1}".FormatWith(Environment.NewLine, package.DocsUrl.ToStringChecked()) : string.Empty,
                                !string.IsNullOrWhiteSpace(package.MailingListUrl.ToStringChecked()) ? "{0} Mailing List: {1}".FormatWith(Environment.NewLine, package.MailingListUrl.ToStringChecked()) : string.Empty,
                                !string.IsNullOrWhiteSpace(package.BugTrackerUrl.ToStringChecked()) ? "{0} Issues: {1}".FormatWith(Environment.NewLine, package.BugTrackerUrl.ToStringChecked()) : string.Empty,
        ...
        public virtual void Pack(ChocolateyConfiguration config)
        {
            ...
            _fileSystem.EnsureDirectory(outputFolder);
...
folder.");
        ...
        public void InstallDryRun(ChocolateyConfiguration config, Action<PackageResult, ChocolateyConfiguration> continueAction)
        {
            ...
            _fileSystem.EnsureDirectory(tempInstallsLocation);
        ...
        {
            _fileSystem.EnsureDirectory(ApplicationParameters.PackagesLocation);
 ...
 Please use upgrade if you meant to upgrade to a new version.".FormatWith(installedPackage.Name, installedPackage.Version));
                ...
                var availablePackage = NugetList.FindPackage(packageName, config, _nugetLogger, sourceCacheContext, NugetCommon.GetRepositoryResource<PackageMetadataResource>(remoteRepositories).ToList(), NugetCommon.GetRepositoryResource<ListResource>(remoteRepositories).ToList(), latestPackageVersion);
...
Please see https://docs.chocolatey.org/en-us/troubleshooting for more
 assistance.");
                ...
                foreach (SourcePackageDependencyInfo packageDependencyInfo in resolvedPackages)
                {
                    ...
                    try
                    {
                        ...
                        var packageResult = packageResultsToReturn.GetOrAdd(packageDependencyInfo.Id.ToLowerChecked(), new PackageResult(packageMetadata, packageRemoteMetadata, installedPath));
        ...
        public virtual ConcurrentDictionary<string, PackageResult> Upgrade(ChocolateyConfiguration config, Action<PackageResult, ChocolateyConfiguration> continueAction, bool performAction, Action<PackageResult, ChocolateyConfiguration> beforeUpgradeAction = null)
        {
            ...
            foreach (string packageName in config.PackageNames.Split(new[] { ApplicationParameters.PackageNamesSeparator }, StringSplitOptions.RemoveEmptyEntries).OrEmpty())
            {
                ...
                var availablePackage = NugetList.FindPackage(packageName, config, _nugetLogger, sourceCacheContext, NugetCommon.GetRepositoryResource<PackageMetadataResource>(remoteRepositories).ToList(), NugetCommon.GetRepositoryResource<ListResource>(remoteRepositories).ToList(), version);
                ...
                if (availablePackage == null)
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            this.Log().Info("{0}|{1}|{1}|{2}".FormatWith(installedPackage.PackageMetadata.Id, installedPackage.Version, isPinned.ToStringChecked().ToLowerChecked()));
                        }
                    }
                ...
                if (installedPackage.PackageMetadata.Version == availablePackage.Identity.Version)
                {
                    ...
                    if (!config.Force)
                    {
                        ...
                        if (!config.UpgradeCommand.NotifyOnlyAvailableUpgrades)
                        {
                            ...
                            {
                                this.Log().Info("{0}|{1}|{2}|{3}".FormatWith(installedPackage.PackageMetadata.Id, installedPackage.Version, availablePackage.Identity.Version, isPinned.ToStringChecked().ToLowerChecked()));
                            }
                        }
                ...
                if ((availablePackage.Identity.Version > installedPackage.PackageMetadata.Version) || config.Force || (availablePackage.Identity.Version < installedPackage.PackageMetadata.Version && config.AllowDowngrade))
                {
                    ...
                    if (performAction)
                    {
                        ...
                        {
                            foreach (var packageVersion in NugetList.FindAllPackageVersions(parentPackage.Id, config, _nugetLogger, sourceCacheContext, NugetCommon.GetRepositoryResource<PackageMetadataResource>(remoteRepositories).ToList()))
                            {
                        ...
                        foreach (SourcePackageDependencyInfo packageDependencyInfo in resolvedPackages)
                        {
                            ...
                            try
                            {
                                ...
                                var upgradePackageResult = packageResultsToReturn.GetOrAdd(packageDependencyInfo.Id.ToLowerChecked(), new PackageResult(packageMetadata, packageRemoteMetadata, installedPath));
        ...
        public virtual ConcurrentDictionary<string, PackageResult> GetOutdated(ChocolateyConfiguration config)
        {
            ...
            foreach (var packageName in packageNames)
            {
                ...
                var latestPackage = NugetList.FindPackage(packageName, config, _nugetLogger, sourceCacheContext, NugetCommon.GetRepositoryResource<PackageMetadataResource>(remoteRepositories).ToList(), NugetCommon.GetRepositoryResource<ListResource>(remoteRepositories).ToList());
                ...
                if (latestPackage == null)
                {
                    ...
                    this.Log().Warn("{0}|{1}|{1}|{2}".FormatWith(installedPackage.Name, installedPackage.Version, isPinned.ToStringChecked().ToLowerChecked()));
                    continue;
                }
                ...
                this.Log().Info("{0}|{1}|{2}|{3}".FormatWith(installedPackage.Name, installedPackage.Version, latestPackage.Identity.Version, isPinned.ToStringChecked().ToLowerChecked()));
            }
        ...
        {
            _fileSystem.EnsureDirectory(ApplicationParameters.PackageBackupLocation);
 ...
 to upgrade. If the package fails to upgrade, this is likely the cause.");
        ...
        {
            if (commandName.ToLowerChecked() == "upgrade")
            {
                ...
                var packagingScripts = _fileSystem.GetFiles(directoryPath, "*.ps1", SearchOption.AllDirectories)
                    .Where(p => filesToDelete.Contains(_fileSystem.GetFilenameWithoutExtension(p).ToLowerChecked()));
        ...
        public virtual void BackupChangedFiles(string packageInstallPath, ChocolateyConfiguration config, ChocolateyPackageInformation packageInfo)
        {
            ...
            var version = packageInfo.Package.Version.ToStringChecked();
        ...
        private void RemovePackageFromCache(ChocolateyConfiguration config, IPackageMetadata installedPackage)
        {
            ...
            var cacheDirectory = _fileSystem.CombinePaths(config.CacheLocation, installedPackage.Id, installedPackage.Version.ToStringChecked());
        ...
        /// </summary>
        /// <param name="installedPackage">The installed package.</param>
        private void RemovePackageFromNugetCache(IPackageSearchMetadata installedPackage)
        {
            ...
            FaultTolerance.TryCatchWithLoggingException(
                () =>
                {
                    var packageFolderPath = _fileSystem.CombinePaths(tempFolder, "{0}/{1}".FormatWith(installedPackage.Identity.Id, installedPackage.Identity.Version.ToStringChecked()));
                    var nugetCachedFile = _fileSystem.CombinePaths(packageFolderPath, "{0}.{1}.nupkg".FormatWith(installedPackage.Identity.Id, installedPackage.Identity.Version.ToStringChecked()));
 ...
 *.install/*.portable package that it installed
 ({0} represents discoverability).".FormatWith(packageName));
        ...
        private void NormalizeNuspecCasing(IPackageSearchMetadata packageMetadata, string packageLocation)
        {
            ...
            var lowercaseNuspec = _fileSystem.CombinePaths(packageLocation, "{0}{1}"
                .FormatWith(packageMetadata.Identity.Id.ToLowerChecked(), NuGetConstants.ManifestExtension));
