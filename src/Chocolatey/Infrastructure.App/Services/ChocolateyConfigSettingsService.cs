...
namespace Chocolatey.Infrastructure.App.Services
{
    ...
    public class ChocolateyConfigSettingsService : IChocolateyConfigSettingsService
    {
            ...
            foreach (var source in ConfigFileSettings.Sources)
            {
                ...
                if (!configuration.QuietOutput)
                {
                    if (configuration.RegularOutput)
                    {
                        ...
                        source.Priority,
                        source.BypassProxy.ToStringChecked(),
                        source.AllowSelfService.ToStringChecked(),
                        source.VisibleToAdminsOnly.ToStringChecked()
                        ));
                    }
                    else
                    {
                        ...
                        source.Value,
                        source.Disabled.ToStringChecked(),
        ...
        {
            if (!_knownFeatures.Contains(name.ToLowerChecked()))
            {
                _knownFeatures.Add(name.ToLowerChecked());
            }
        }

        protected void ValidateSupportedFeature(ConfigFileFeatureSetting feature)
        {
            if (!_knownFeatures.Contains(feature.Name.ToLowerChecked()))
            {
