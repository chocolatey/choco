    ...
    public sealed class RegistryService : IRegistryService
    {
        ...
        private RegistryKey OpenKey(RegistryHive hive, RegistryView view)
        {
            return FaultTolerance.TryCatchWithLoggingException(
                 ...
                 "Could not open registry hive '{0}' for view '{1}'".FormatWith(hive.ToStringChecked(), view.ToStringChecked()),
        ...
        public Registry GetInstallerKeys()
        {
            ...
            if (windowsIdentity != null) snapshot.User = windowsIdentity.User.ToStringChecked();
        ...
        public void UpdateSnapshot(RegistryKey key, Registry snapshot)
        {
            ...
            if (!string.IsNullOrWhiteSpace(appKey.DisplayName))
            {
                ...
                if (appKey.WindowsInstaller || appKey.UninstallString.ToStringChecked().ToLowerChecked().Contains("msiexec"))
                {
                ...
                if (appKey.InstallerType == InstallerType.Unknown && appKey.UninstallString.ToStringChecked().ToLowerChecked().Contains("uninst.exe"))
                {
                    appKey.InstallerType = InstallerType.Nsis;
                }
                ...
                if (_logOutput)
                {
                    ...
                    //{
                        foreach (var name in key.GetValueNames())
                        {
                            ...
                            {
                                Console.WriteLine("key - {0}|{1}={2}|Type detected={3}|install location={4}".FormatWith(key.Name, name, value.ToStringSafe(), appKey.InstallerType.ToStringChecked(),appKey.InstallLocation.ToStringChecked()));
                            }
        ...
        private string GetMsiUserDataKey(string name)
        {
            ...
            return _userDataKey.ToStringChecked();
        }
                ...
                if (subKey != null)
                {
                    foreach (var valueName in subKey.GetValueNames())
                    {
                        values.Add(new GenericRegistryValue
                        {
                            ...
                            Type = (RegistryValueKindType)Enum.Parse(typeof(RegistryValueKindType), subKey.GetValueKind(valueName).ToStringChecked(), ignoreCase: true),
                            Value = subKey.GetValue(valueName, expandValues ? RegistryValueOptions.None : RegistryValueOptions.DoNotExpandEnvironmentNames).ToStringChecked().Replace("\0", string.Empty),
                        });
                    }
                }
            }
        ...
        {
            var hiveActual = (RegistryHive)Enum.Parse(typeof(RegistryHive), hive.ToStringChecked(), ignoreCase: true);
            IList<RegistryKey> keyLocations = new List<RegistryKey>();
            if (Environment.Is64BitOperatingSystem)
            {
                            ...
                            () =>
                            {
                                if (key.GetValueNames().Contains(registryValue, StringComparer.InvariantCultureIgnoreCase))
                                {
                                    return new GenericRegistryValue
                                    {
                                        ...
                                        Type = (RegistryValueKindType)Enum.Parse(typeof(RegistryValueKindType), key.GetValueKind(registryValue).ToStringChecked(), ignoreCase: true),
