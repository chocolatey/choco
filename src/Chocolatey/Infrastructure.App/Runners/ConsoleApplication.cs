            ...
            runner.Run(config, container, isConsole: true, parseArgs: command =>
                {
                    ConfigurationOptions.ParseArgumentsAndUpdateConfiguration(
                        ...
                        (unparsedArgs) => {
                            ...
                            if (config.Debug) Log4NetAppenderConfiguration.EnableDebugLoggingIf(config.Debug, "{0}LoggingColoredConsoleAppender".FormatWith(ChocolateyLoggers.Verbose.ToStringChecked()), "{0}LoggingColoredConsoleAppender".FormatWith(ChocolateyLoggers.Trace.ToStringChecked()));
