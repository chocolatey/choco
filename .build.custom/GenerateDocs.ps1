...
$lineFeed = "`r`n"
$global:powerShellReferenceTOC = "# Chocolatey Package Functions aka Helpers Reference$lineFeed$lineFeed"
...
function Convert-CommandHeadings {
param(
  [string]$commandText,
  [string]$commandName = ''
)
...
function Generate-TopLevelCommandReference {
  ...
  $commandOutput | %{ Convert-CommandHeadings($_) } | %{ Convert-CommandReferenceSpecific($_) } | Out-File $fileName -Encoding UTF8 -Force
}
...
function Generate-CommandReference($commandName) {
  ...
  $commandOutput | %{ Convert-CommandHeadings $_ $commandName } | Out-File $fileName -Encoding UTF8 -Force
}
